...
@section('content')
    <main class="p-2 bg-Neutral-10">
        <section class="p-4 md:p-20 flex flex-col gap-6 rounded-xl border border-Neutral-10 bg-Neutral-0 h-[calc(88vh)] md:h-[calc(87vh)]"> {{-- Outer Card --}}
            <div class="p-4 flex flex-col gap-4 justify-between md:justify-normal rounded-xl border border-Neutral-10 md:w-[480px] h-lvh md:h-fit self-center fadeIn"> {{-- Inner Card --}}
                <div class="flex flex-col gap-12">
                    <div class="flex flex-col gap-2 text-center">
                        <p class="cardTitle">
                            @if ($bansos->isEmpty())
                                Anda bukan penerima bantuan sosial
                            @else
                    ...
                    @if ($bansos->isEmpty())
                        <img src="{{ asset('assets/bansos_false.png') }}" alt=":(" class="h-60 w-60 flex self-center">
                    @else
                    <div class="p-3 flex flex-col gap-3 bg-Neutral-10 rounded-lg">
                        @foreach ($bansos as $item)   
                            <div class="flex gap-3 items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 20 20">
                                    <path fill="#fff" d="M8.594 13.412a.831.831 0 0 1-.59-.244l-2.343-2.344a.833.833 0 0 1 1.178-1.178L8.594 11.4l4.567-4.567a.833.833 0 0 1 1.178 1.178l-5.156 5.157a.83.83 0 0 1-.59.244Z" opacity=".99"/>
                                    <path fill="#1B1B1B" d="M10 1.667A8.333 8.333 0 1 0 18.333 10 8.343 8.343 0 0 0 10 1.667Zm4.34 6.344-5.157 5.157a.833.833 0 0 1-1.178 0L5.66 10.824a.833.833 0 0 1 1.178-1.178L8.594 11.4l4.567-4.567a.833.833 0 0 1 1.178 1.178Z"/>
                                </svg>
                                <p class="text-base md:text-lg font-medium">{{ $item->bansos->bansos_nama }}</p>
                            </div>
                        @endforeach
                    </div>
                    @endif
